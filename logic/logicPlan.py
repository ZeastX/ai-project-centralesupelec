...
def sentence2() -> Expr:
    ...
    expr3=(~(B&(~C)))>>A
    expr4=(~D)>>C
    expr = conjoin(expr1,expr2,expr3,expr4)
    return expr
    "*** END YOUR CODE HERE ***"
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    inv = ~inverse_statement
    return pl_true(inv,assignments)
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    expressions = list(itertools.combinations(literals,2))
    expr = [~(expressions[0][0] & expressions[0][1])]
    for i in range(1,len(expressions)):
        expr.append(~(expressions[i][0] & expressions[i][1]))
    expr = conjoin(expr)
    return to_cnf(expr)
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    expr = disjoin(possible_causes)
    
    return PropSymbolExpr(pacman_str,x,y,time=now) % expr
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x,y in non_outer_wall_coords:
        pacpos.append(PropSymbolExpr(pacman_str,x,y,time=t))
    pacphysics_sentences.append(exactlyOne(pacpos))
    pacphysics_sentences.append(exactlyOne([PropSymbolExpr('North',time=t),PropSymbolExpr('South',time=t),PropSymbolExpr('East',time=t),PropSymbolExpr('West',time=t)]))
    if sensorModel != None:
    ...
    if successorAxioms != None:
        ...
        if t!=0:
            Succ = successorAxioms(t,walls_grid,non_outer_wall_coords)    
            pacphysics_sentences.append(Succ)
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    mod1 = conjoin(KB)&PropSymbolExpr(pacman_str,x1,y1,time=1)
    mod2 = conjoin(KB)&(~PropSymbolExpr(pacman_str,x1,y1,time=1))
    print(findModel(mod1))
    return findModel(mod1),findModel(mod2)
    "*** BEGIN YOUR CODE HERE ***"
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        expr = goalAssertion & conjoin(KB)
        model = findModel(expr)
        if model != False:
            return extractActionSequence(model,actions)
        act = []
        for a in actions:
            act.append(PropSymbolExpr(a,time=t))
...
def foodLogicPlan(problem) -> List:
    ...
    expr = []
    for x,y in food:
        expr.append(PropSymbolExpr(food_str,x,y,time=0))
    foods.append(conjoin(expr))
    KB.append(foods[0])
    for t in range(50):
        print(t)
        coords = []
        ...
        expr = conjoin(conjoin(goalAssertion),conjoin(KB))
        model = findModel(expr)
        print(model)
        if model != False:
            return extractActionSequence(model,actions)
        ...
        for i in range(len(food)):
            x,y = food[i]
            foods.append(PropSymbolExpr(food_str,x,y,time=t+1))
        print(len(foods))
        expr1 = atLeastOne(foods)
        x,y = food[0]
        expr = [~PropSymbolExpr(food_str,x,y,time=t+1)]
        for i in range(1,len(food)):
            x,y = food[i]
            expr.append(~PropSymbolExpr(food_str,x,y,time=t+1))
        expr2 = conjoin(expr)
        expression = disjoin(expr1,expr2)
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        a = agent.actions[t]
        KB.append(PropSymbolExpr(a,time=t))
        percepts = fourBitPerceptRules(t,agent.getPercepts())
        KB.append(percepts)
        for x,y in non_outer_wall_coords:
            poss = []
            imposs = []
            sat = findModel(conjoin(KB) & PropSymbolExpr(pacman_str,x,y,time=t))
            if sat != False:
                poss.append(PropSymbolExpr(pacman_str,x,y,time=t))
                possible_locations.append((x,y))
                print(possible_locations)
            else:
                imposs.append(~PropSymbolExpr(pacman_str,x,y,time=t))
        agent.moveToNextState(a)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x,y in non_outer_wall_coords:
            poss = []
            imposs = []
            sat = entails(conjoin(KB) , PropSymbolExpr(wall_str,x,y))
            sat2 = entails(conjoin(KB) , ~PropSymbolExpr(wall_str,x,y))
            if sat:
                KB.append(PropSymbolExpr(wall_str,x,y))
                known_map[x][y] = 1
            elif sat2:
